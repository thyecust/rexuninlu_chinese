...
class RotaryEmbedding(nn.Module):
    ...
    def rotate_queries_or_keys(self, t, seq_dim = -2, position_ids=None):
        assert not self.use_xpos, 'you must use `.rotate_queries_and_keys` method instead and pass in both queries and keys, for length extrapolatable rotary embeddings'
        device, seq_len = t.device, t.shape[seq_dim]
        freqs = self.forward(lambda: torch.arange(seq_len, device = device) if position_ids is None else position_ids)
